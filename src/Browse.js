  ...
  const filterCatalog = (search) => {
    ...
    const updatedCatalog = catalog.filter((item) => {
      for (let term of searchTerms) {
        if (
          !item.name.toLowerCase().trim().includes(term.toLowerCase().trim())
        ) {
          return false;
        }
      }
  ...
  const shoppingItems = viewCatalog.map((item) => (
    <div className="col justify-content-center" key={item.id}>
      <div className="card w-100 h-100">
        <div className="card-header p-0 bg-white align-content-center">
          <img
            src={item.image.url}
            className="card-img-top img-fluid object-fit-contain p-2"
            style={{ maxHeight: "24rem"}}
            alt={`Image of ${item.name}, ${item.image.author}\nSource: ${item.image.source}`}
          />
        </div>
